...
[assembly: AssemblyFileVersion("2023.09.12.0")]
...
namespace J_RFID
{
	public class RFIDAPI
	{
		...
		public byte HF_Receive(out string Data)
		{
			Data = "";
			try
			{
		...
		{
			strVersion = "V23.09.12";
			return 0;
		}

		public byte RFID_W_ReaderSN(string SN)
		{
			string strRece = "";
			try
			{
				if (SN.Length == 8)
				{
					char[] array = SN.ToCharArray(0, 8);
					if (Uri.IsHexDigit(array[0]) && Uri.IsHexDigit(array[1]) && Uri.IsHexDigit(array[2]) && Uri.IsHexDigit(array[3]) && Uri.IsHexDigit(array[4]) && Uri.IsHexDigit(array[5]) && Uri.IsHexDigit(array[6]) && Uri.IsHexDigit(array[7]))
					{
						ComSend("010D000304E205" + SN + "0000", out strRece, 50);
						return 0;
					}
					return 6;
				}
		...
		public byte RFID_R_ReaderSN(out string SN)
		{
			SN = "";
			string strRece = "";
			try
			{
				ComSend("0108000304E30002", out strRece, 50);
				SN = strRece.Substring(18, 8);
				return 0;
			}
			catch
			{
			...
			try
			{
				...
				case 0:
					if (Power == 1)
					{
						...
						num = ComSend("010C00030410002101000000", out strRece, 200);
						if (num != 0)
						{
							return 4;
						}
		...
		public byte RFID_ISO15693Inventory(string Flag, string Afi, string Timeout, out string Uid, out string Rssi)
		{
			int num = 0;
			int num2 = 100;
			string strRece = "";
				...
				if (strRece.Length >= 1)
				{
					...
					for (int i = 0; i < array.Length; i++)
					{
						if (array[i] == '[' && array[i + 1] != ',' && array[i + 2] != ',' && array[i + 1] != ']')
						{
							...
							if (i + 19 < array.Length)
							{
								Rssi = Rssi + array[i + 18].ToString() + array[i + 19].ToString();
							}
		...
		public byte RFID_ISO15693Read_mult(string Flag, string Uid, string Block, string length, out string Data)
		{
			string strRece = "";
			string text = "";
			if (length.Length < 2)
			...
			if (Flag.Length == 2)
			{
				try
				{
					...
					ComSend("011400030418" + Flag + "23" + text + Block + length + "0000", out strRece, 120);
					char[] array = strRece.ToCharArray();
					int num = 0;
					int num2 = 0;
					for (int i = 0; i < array.Length; i++)
					{
						...
						if (num2 != 0)
						{
							...
							if (text2.Length > 10)
							{
								Data = text2.Substring(3, text2.Length - 4);
								return 0;
							}
							num2 = 0;
						}
		...
		public byte RFID_ST25DVRead(string Flag, string Uid, string Block, out string Data)
		{
			string strRece = "";
			string text = "";
			if (Flag.Length < 2)
			...
			if (Flag.Length == 2)
			{
				try
				{
					...
					if (text.Length == 16)
					{
						ComSend("011400030418" + Flag + "30" + text + Block + "0000", out strRece, 100);
					}
					else
					{
						ComSend("010C00030418" + Flag + "30" + Block + "0000", out strRece, 100);
					}
					char[] array = strRece.ToCharArray();
					int num = 0;
					int num2 = 0;
		...
		public byte RFID_ST25DVWrite(string Flag, string Uid, string Block, string Data)
		{
			string strRece = "";
			string text = "";
			string text2 = "";
			...
			if (Flag.Length == 2)
			{
				...
				try
				{
					...
					if (text2.Length == 16)
					{
						ComSend("011800030418" + Flag + "31" + text2 + text + Data + "0000", out strRece, 100);
					}
					else
					{
						ComSend("011000030418" + Flag + "31" + text + Data + "0000", out strRece, 100);
					}
					if (strRece.IndexOf("[00]") <= -1)
					{
						return 1;
			...
			try
			{
					...
					if (Key.Length == 12)
					{
						if (!(Firmware_mode != "FD"))
						{
							...
							if (num == 0)
							{
								...
								else if (Uid14443A.Length == 18)
								{
									num = ComSend("0111000304A2" + Uid14443A + "0000", out strRece, 200);
									if (num != 0)
									{
										return 4;
									}
								...
								if (num == 0)
								{
									...
									case 0:
										...
										{
											num = ComSend("0113000304C160" + text + Uid14443A + "0000", out strRece, 200);
											if (num != 0)
											{
												return 4;
											}
									...
									case 1:
										...
										{
											num = ComSend("0113000304C161" + text + Uid14443A + "0000", out strRece, 40);
											if (num != 0)
											{
												return 4;
											}
				...
				case 1:
					...
					if (num == 0)
					{
						...
						{
							num = ComSend("0111000304A2" + Uid14443A + "0000", out strRece, 40);
						}
						if (num == 0)
						{
							...
							if (num == 0)
							{
								...
								case 0:
									...
									{
										num = ComSend("0113000304C160" + text + Uid14443A + "0000", out strRece, 40);
									}
									if (num != 0)
									{
										return 4;
				...
				if (Key.Length == 12)
				{
					if (!(Firmware_mode != "FD"))
					{
						...
						if (num == 0)
						{
							...
							{
								num = ComSend("0111000304A2" + UID + "0000", out strRece, 60);
								if (num != 0)
								{
									return 4;
								}
							...
							if (num == 0)
							{
								...
								case 0:
									...
									{
										num = ComSend("0113000304C160" + text + UID + "0000", out strRece, 40);
										if (num != 0)
										{
											return 4;
										}
								...
								case 1:
									...
									{
										num = ComSend("0113000304C161" + text + UID + "0000", out strRece, 40);
										if (num != 0)
										{
											return 4;
										}
				...
				try
				{
					...
					case 0:
						if (!(Firmware_mode != "FD"))
						{
							...
							if (num == 0)
							{
								...
								{
									num = ComSend("0111000304A2" + Uid14443A + "0000", out strRece, 60);
									if (num != 0)
									{
										return 4;
									}
					...
					case 1:
						...
						if (num == 0)
						{
							...
							{
								num = ComSend("0111000304A2" + Uid14443A + "0000", out strRece, 70);
								if (num != 0)
								{
									return 4;
								}
							...
							if (num == 0)
							{
								...
								case 0:
									...
									{
										num = ComSend("0113000304C160" + text + Uid14443A + "0000", out strRece, 70);
										if (num != 0)
										{
											return 4;
										}
								...
								case 1:
									...
									{
										num = ComSend("0113000304C161" + text + Uid14443A + "0000", out strRece, 70);
										if (num != 0)
										{
											return 4;
										}
		...
		public byte RFID_Get14443AUID(string Antenna_ON_OFF, out string Uid)
		{
			string strRece = "";
			long num = 0L;
			int num2 = 0;
				...
				if (num == 0)
				{
					...
					if (num2 >= 1 && num3 >= 1)
					{
						...
						if (Uid14443A.Length >= 1)
						{
							...
							if (num == 0)
							{
								num = ComSend("01" + text + "000304A2" + Uid + "0000", out strRece, 80);
								if (num == 0)
								{
									num5 = strRece.IndexOf("[");
									num6 = strRece.IndexOf("]");
									if (num6 - num5 == 3)
									{
										if (Uid.Length >= 4)
										{
		...
		public byte RFID_GetS50UID(string Antenna_ON_OFF, out string Uid, out string Ctype)
		{
			string strRece = "";
			string text = "";
			long num = 0L;
						...
						if (num == 0)
						{
							...
							if (num == 0)
							{
								...
								if (num6 - num5 == 3)
								{
									if (Uid.Length >= 4)
									{
										Uid = Uid.Substring(6, 2) + Uid.Substring(4, 2) + Uid.Substring(2, 2) + Uid.Substring(0, 2);
										if (Antenna_ON_OFF == "1")
										{
											ComSend("010A0003041000800000", out strRece, 20);
											if (Power_set == 1)
		...
		public byte RFID_ReadUltraLightBlock(string Block, out string Data)
		{
			...
			if (Block.Length >= 2)
			{
				try
				{
					ComSend("0108000304FD0000", out strRece, 200);
					Firmware_mode = "FD";
					ComSend("010A0003041000010000", out strRece, 200);
		...
		public byte Card_EmulationA(string ID)
		{
			string text = "";
			if (ID.Length != 8)
			{
			...
			sptCom.Write("010D000304CE00" + ID + "0000");
			Thread.Sleep(80);
			text = sptCom.ReadExisting();
			if (text.IndexOf("Tag emulation") > -1)
			{
				return 0;
			}
			if (text.IndexOf("tag emulation") > -1)
		...
		public byte Card_EmulationB(string ID)
		{
			string text = "";
			if (ID.Length != 8)
			{
			...
			sptCom.Write("010D000304CE01" + ID + "0000");
			Thread.Sleep(80);
			text = sptCom.ReadExisting();
			if (text.IndexOf("Tag emulation") > -1)
			{
